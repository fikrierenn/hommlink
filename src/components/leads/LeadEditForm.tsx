...
interface LeadEditFormProps {
  lead: Lead
  onSave: (updates: LeadUpdate) => Promise<void>
  onCancel: () => void
}
...
export function LeadEditForm({ lead, onSave, onCancel }: LeadEditFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    next_action_at: lead.next_action_at ? lead.next_action_at.slice(0, 16) : '' // Format for datetime-local input
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updates: LeadUpdate = {
        ...
        next_action_at: formData.next_action_at ? new Date(formData.next_action_at).toISOString() : undefined
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Durum ve Takip</h2>
          ...
          <Input
            ...
            placeholder="Örn: Randevu planla"
          />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleInputChange('next_action_at', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary"
            />
          </div>
